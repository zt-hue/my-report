...
def list_available_skills():
    """
    读取项目目录下 .agents/skills 目录中的所有一级子目录，
    读取每个子目录下的 SKILL.md 文件的 YAML front matter，
    ...
    try:
        ...
        if not os.path.exists(skills_dir):
            return json.dumps({"status": "success", "skills": []}, ensure_ascii=False)
        
        skills = []
        
        # 遍历 skills 目录下的所有一级子目录
        for skill_name in os.listdir(skills_dir):
            ...
            if len(lines) > 0 and lines[0].strip() == '---':
                ...
                for line in lines[1:]:
                    if line.strip() == '---':
                        break
                    if ':' in line and in_front_matter:
                        key, value = line.split(':', 1)
...
def load_skill_content(skill_name):
    ...
    try:
        # 获取项目根目录
        project_root = os.path.dirname(os.path.dirname(__file__))
        skill_dir = os.path.join(project_root, '.agents', 'skills', skill_name)
        ...
        skill_file = os.path.join(skill_dir, 'SKILL.md')
        if not os.path.exists(skill_file):
            return json.dumps({"status": "error", "message": f"技能 {skill_name} 的 SKILL.md 文件不存在"}, ensure_ascii=False)
        ...
        return json.dumps({"status": "success", "data": body_content}, ensure_ascii=False)
    except Exception as e:
        return json.dumps({"status": "error", "message": str(e)}, ensure_ascii=False)
    ...
    tools = [
        ...
        },
        {
            ...
            "function": {
                ...
                "description": "列出当前可用的技能列表，包括技能名称和描述",
                "parameters": {
                    "type": "object",
                    "properties": {}
                }
        ...
        },
        {
            ...
            "function": {
                ...
                "description": "加载指定技能的详细内容，当用户请求涉及该技能的场景时使用",
                "parameters": {
                    "type": "object",
                    "properties": {
                        "skill_name": {
...
9. 列出技能：使用 list_available_skills 工具列出可用技能列表
10. 加载技能：使用 load_skill_content 工具加载指定技能的详细内容

## 技能调用规则
1. 分析用户需求，判断是否需要使用某个技能
2. 如果需要使用技能，先调用 load_skill_content 工具加载该技能的详细内容
3. 根据技能内容指导你的回答
4. 如果用户的请求涉及多个技能，可以逐一加载并应用
...
当使用 load_skill_content 工具加载技能内容后，你需要遵照技能内容进行回答。"""
    ...
    print("\n=== LLM 工具调用客户端 (practice06) ===")
    print("输入消息开始聊天，按 Ctrl+C 退出")
    print("=====================================")
    print("可用技能：", ", ".join([skill.get('name') for skill in skills_list]))
    print("可用工具：list_files, rename_file, delete_file, create_file, read_file, fetch_webpage, search_chat_history, anythingllm_query, list_available_skills, load_skill_content")
    print()
    print("示例：")
    print("1. 列出目录内容：列出当前目录的文件")
    print("2. 访问网页：访问 https://www.example.com 并总结内容")
    print("3. 搜索聊天历史：/search 我之前说了什么")
    print("4. 使用技能：帮我写一份五一放假通知")
    print()
